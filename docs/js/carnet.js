...
export const EQUIPE = { personnes: 3, joursParMoisEtParPersonne: 16 };
/** Jours ouvrés que l'équipe produit par jour de calendrier ouvré. */
export function capaciteParJour(e) {
 ...
 */
export function planifier(ordre, carnet, equipe, estimation) {
...
export const POLITIQUES = {
    ...
    graviteDabord: (c) => [...c].sort((a, b) => b.coutParMoisDeRetard - a.coutParMoisDeRetard).map((x) => x.ref),
    /** L'échéance la plus proche d'abord. */
    echeanceDabord: (c) => [...c].sort((a, b) => a.echeance - b.echeance).map((x) => x.ref),
    /** Le plus court d'abord : on vide la liste vite, ce qui plaît au comité. */
    plusCourtDabord: (c) => [...c].sort((a, b) => a.charge.centre - b.charge.centre).map((x) => x.ref),
    /**
